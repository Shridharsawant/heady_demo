...
//  UTGo
//
//  Created by Shridhar Sawant on 22/09/17.
//  Copyright © 2017 Plexitech. All rights reserved.
//

import UIKit

@objc protocol RateViewDelegate : class {
    func rateViewDidChangeRating(rateView : RateView, rating : Int)
}

class RateView: UIView {
    
    let unselectedImage = UIImage(named: "rate_star_unselected")
    let selectedImage = UIImage(named: "rate_star_selected")
//    let selectedGreenImage = UIImage(named: "rate_green_star_selected")
    ...
    @objc var delegate : RateViewDelegate?
...
//        let imageHeight = max(minImageSize.height, frame.height)
        ...
        for i in 0..<imageViews.count {
            ...
            imageView.frame = CGRect(x: sideMargin + (CGFloat(i) * (imageWidth + midMargin)), y: 0, width: imageWidth, height: frame.height)
